...
import { purchaseReturnData } from '../../utils/data/purchaseData';
...
const PurchaseReturn = (props: purchaseReturnProps) => {
    const navigation=useNavigation( );
    const [purchaseOrderFilter, setPurchaseOrderFilter] = useState(false);


    const addPress=()=>{
        navigation.navigate(screenName.AddPurchaseReturn,{isAddedProductAvailable:false} as never)
    }
    ...
    return (
        ...
        <View style={mh15}>
            <View style={mv15}>
                <TopHeader headerText={labels.purchaseReturn} searchIcon={true} searchName={'Search' + labels.purchaseReturn} searchText={labels.purchaseReturn} />
            </View>
            <BottomWidth style={[mv5]} />
            <ScrollView>
                <View style={{ paddingBottom: '50%' }}>
                    <ListSubHeader listName={labels.purchaseReturn} totalNumber={'21'} addIcon={true} onAddPress={addPress} onFilterPress={openFilterModal} />
        ...
        <BottomNavBar />
        <CustomModal children={<PurchaseReturnFilter onSave={addSaveBtn} onCancel={closeModal}   />} visible={purchaseOrderFilter} onClose={closeModal} height={'80%'} />
